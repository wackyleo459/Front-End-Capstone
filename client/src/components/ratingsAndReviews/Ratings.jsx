...
class Ratings extends React.Component {
  ...
  getRating(event) {
    axios
      .get(`http://localhost:3000/reviews/?product_id=${this.state.id}`)
      .then((res) => {
        this.setState({
  ...
  getRatingMeta(event) {
    axios
      .get(`http://localhost:3000/reviews/meta/?product_id=${this.state.id}`)
      .then((res) => {
        this.setState({
  ...
  fillRatingStars(num) {
    ...
    for (let i = 0; i < num; i++) {
      ...
      if (num % 1 !== 0) {
        rating[Math.floor(Math.abs(num))] = (
          <i key={i * 10} className="fas fa-star-half-alt"></i>
        );
      }
    }
    return rating;
  ...
  getAverageRating(list) {
    const avg =
      list.reduce((avg, review) => (avg += review.rating), 0) /
      ...
      <div className="column1Nd">
        <div className="ratingsNd">
          <StarRatingEntry
            ...
            stars={this.fillRatingStars(
              this.getAverageRating(this.state.reviews)
            )}
