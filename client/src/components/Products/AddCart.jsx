...
const AddCart = ({ skus }) => {
  ...
  const buttonHide = () => {
    if (!qty) {
      //refactor to check if ALL SIZES for one style is out of stock
      return { display: "none" };
    }
  };
  ...
  return (
    <div className="add">
      <form id="selectForm" style={{ display: "inline-flex" }}>
        ...
        <label id="quantity">
          ...
          {isSizeSelected ? (
            ...
            <select id="selectQty" disabled={true}>
              <option>-</option>
            </select>
          )}
        </label>
      ...
      <span className="cartItems" style={!cart ? { display: "none" } : null}>
        {cart}
      </span>
    </div>
  );
